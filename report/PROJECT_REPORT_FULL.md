# Bank Churn Statistical Analysis — Báo cáo Markdown chi tiết (Full Project Report)

**Repository**: `bank-churn-statistical-analysis`
...
## Lưu ý về hiển thị công thức (Math rendering note)

Các công thức trong báo cáo dùng LaTeX math syntax với delimiter `$...$` (inline) và `$$...$$` (block).

- Nếu bạn xem bằng renderer **có hỗ trợ KaTeX/MathJax** (ví dụ GitHub Markdown), công thức sẽ render bình thường.
- Nếu bạn xem bằng Markdown preview **không hỗ trợ math**, bạn sẽ thấy nguyên văn `$$...$$` — đây không phải lỗi nội dung, mà là giới hạn của renderer.

## 0) Mục tiêu báo cáo (Goal)

Mục tiêu của báo cáo này là cung cấp một tài liệu Markdown **đủ chi tiết để trình bày cho giáo viên**, bao gồm:

- Công thức toán học (mathematical formulas - công thức toán) của Logistic Regression (hồi quy logistic) và Odds Ratio (tỷ số odds).
- Quy trình làm việc (workflow - quy trình) end-to-end: data → preprocess → train → evaluate → interpret.
- Trích dẫn cụ thể **file code + line numbers** nơi mô hình/pipeline/cách tính được triển khai.
- Kết quả thực nghiệm hiện tại (metrics + OR) và diễn giải.
- Cấu trúc dự án và cách chạy lại trong conda env `data_science`.

## 1) Tóm tắt đề tài (Project summary)

- **Đề tài**: Nghiên cứu các yếu tố ảnh hưởng đến quyết định rời bỏ của khách hàng ngân hàng (churn) bằng Logistic Regression.
...
```text
bank-churn-statistical-analysis/
  data/                       # dữ liệu gốc
  ...
  src/bank_churn/             # package: logic phân tích/model/reporting
...
Các module quan trọng trong `src/bank_churn/`:

- **FeatureSpec** (đặc tả biến): `src/bank_churn/preprocessing/features.py` (L6–L22)
- **Preprocessing** (tiền xử lý): `src/bank_churn/preprocessing/pipeline.py` (L20–L37)
- **Train/Test split + Pipeline**: `src/bank_churn/training/pipeline.py` (L22–L35)
- **Logistic model** (sklearn): `src/bank_churn/model/logistic.py` (L8–L31)
- **Evaluation metrics**: `src/bank_churn/evaluation/metrics.py` (L16–L37)
- **Statsmodels Logit + Odds Ratio**: `src/bank_churn/stats/logit_odds_ratio.py` (L21–L64)
...
- Tách $X, y$: `src/bank_churn/preprocessing/pipeline.py` (hàm `split_xy`, L40–L57)
  - Loại bỏ `id_columns` và `target_column`.
  - Kiểm tra thiếu cột bắt buộc trước khi train (L42–L53).
- Đặc tả biến: `src/bank_churn/preprocessing/features.py` (L6–L22)
  - `target_column="Exited"` (L11)
  - categorical: `("Geography", "Gender")` (L12)
  - numeric: 8 biến (L13–L22)
- Preprocessor:
  - OneHotEncoder cho categorical và StandardScaler cho numeric:
    `src/bank_churn/preprocessing/pipeline.py` (hàm `build_preprocessor`, L20–L37)

### 3.3 Train → Evaluate

- Split train/test có phân tầng (stratified split - chia có phân tầng):
  `src/bank_churn/training/pipeline.py` (hàm `stratified_split`, L22–L26)
- Xây pipeline sklearn:
  `src/bank_churn/training/pipeline.py` (hàm `build_sklearn_pipeline`, L29–L35)
...
- Logistic model spec + instantiate:
  `src/bank_churn/model/logistic.py` (L8–L31)
...
- Evaluation metrics:
  `src/bank_churn/evaluation/metrics.py` (hàm `compute_metrics`, L28–L37)
...
- Chuẩn bị design matrix (ma trận thiết kế): `src/bank_churn/stats/logit_odds_ratio.py` (L21–L35)
...
- Fit Logit và chuyển hệ số sang Odds Ratio:
  `src/bank_churn/stats/logit_odds_ratio.py` (L38–L64)
...
- Mô hình LogisticRegression của sklearn được tạo tại `src/bank_churn/model/logistic.py` (L22–L31).
- Pipeline ghép preprocess + model: `src/bank_churn/training/pipeline.py` (L29–L35).
...
`src/bank_churn/model/logistic.py` (L13–L17, L24–L31).
...
- Trong dự án này: `C=1.0` được đặt tại `src/bank_churn/model/logistic.py` (L15–L16, L22–L31).
...
- p-value và CI được lấy từ statsmodels (MLE + Wald z-test) trong `outputs/logit_summary.txt`, được sinh bởi:
  `src/bank_churn/stats/logit_odds_ratio.py` (L45–L52) và được lưu bởi `scripts/run_pipeline.py` (L167–L174).
...
- OR được tính bằng `np.exp(params.values)` tại:
  `src/bank_churn/stats/logit_odds_ratio.py` (L54–L60)
...
- exponentiate để ra CI của OR: `np.exp(conf["ci_lower"])`, `np.exp(conf["ci_upper"])` (L58–L59)
  trong `src/bank_churn/stats/logit_odds_ratio.py` (L49–L60)

### 5.3 Baseline (nhóm tham chiếu) khi dummy hoá

Vì dùng `drop_first=True`:

- Mỗi biến dummy (ví dụ `Geography_Germany`) được hiểu **so với baseline** của cột đó (category bị drop).
- Quy tắc này được tạo tại:
  `src/bank_churn/stats/logit_odds_ratio.py` (L29–L31)
...
- **Geography**: có `Geography_Germany` và `Geography_Spain` ⇒ baseline là **France**.
- **Gender**: có `Gender_Male` ⇒ baseline là **Female**.

Khi diễn giải chuẩn học thuật, cần viết rõ “so với ai”:

> Ví dụ: `Geography_Germany` có OR=2.17 nghĩa là khách hàng ở Đức có odds churn cao gấp 2.17 lần so với khách hàng ở Pháp (nhóm tham chiếu), trong điều kiện các biến khác không đổi (ceteris paribus).
...
- Recall ~ 0.70: bắt được ~70% khách churn thật (giảm FN).
...
- **IsActiveMember**: OR=0.3411 → khách active giảm odds churn mạnh.
- **Geography_Germany**: OR=2.1700 → odds churn của khách ở Đức cao hơn **so với France (baseline)**, giữ nguyên các biến khác (ceteris paribus).
- **Gender_Male**: OR=0.5895 → odds churn của khách nam thấp hơn **so với Female (baseline)**, giữ nguyên các biến khác (ceteris paribus).
...
- Hệ số chặn $\beta_0$ (const) là **log-odds churn** của “khách hàng cơ sở” khi tất cả biến độc lập bằng 0:
  - Geography baseline: France (vì `Geography_Germany=0`, `Geography_Spain=0`)
  - Gender baseline: Female (vì `Gender_Male=0`)
  - Các biến nhị phân như `IsActiveMember`, `HasCrCard` bằng 0
  - Các biến liên tục (`Age`, `Balance`, ...) bằng 0 (mang tính quy ước toán học; không phải khách hàng “thực tế”)
...
-3.3923
-0.0007 \cdot \text{CreditScore}
+0.0727 \cdot \text{Age}
-0.0159 \cdot \text{Tenure}
+2.637\times 10^{-6} \cdot \text{Balance}
-0.1015 \cdot \text{NumOfProducts}
-0.0447 \cdot \text{HasCrCard}
-1.0754 \cdot \text{IsActiveMember}
+4.807\times 10^{-7} \cdot \text{EstimatedSalary}
...
2) **Mô tả dữ liệu**: 10k khách, churn ~20%, biến numeric/categorical; nêu FeatureSpec:
   `src/bank_churn/preprocessing/features.py` (L6–L22).
3) **EDA**: churn theo Geography/Gender + phân phối Age/Balance/CreditScore/Salary; nêu file EDA:
   `src/bank_churn/analysis/eda.py` (L20–L41) và hình `figures/*.png`.
4) **Tiền xử lý**: OneHotEncoder + StandardScaler:
   `src/bank_churn/preprocessing/pipeline.py` (L20–L37).
5) **Train/test**: stratified split:
   `src/bank_churn/training/pipeline.py` (L22–L26).
6) **Mô hình**: LogisticRegression (L2 regularization, class_weight balanced):
   `src/bank_churn/model/logistic.py` (L8–L31) và `scripts/run_pipeline.py` (L107–L111).
7) **Đánh giá**: compute_metrics và confusion matrix:
   `src/bank_churn/evaluation/metrics.py` (L28–L37); nêu ROC AUC/Recall.
8) **Suy luận thống kê**: Logit + OR=exp(beta):
   `src/bank_churn/stats/logit_odds_ratio.py` (L38–L64).
9) **Khuyến nghị**: tập trung tăng active membership, phân tích theo geography, tối ưu threshold theo chi phí.

## 8) Cách chạy lại (Reproducibility)

### 8.1 Chạy Python pipeline trong conda env `data_science`
...
- **Metrics JSON**: `scripts/run_pipeline.py` (L122–L154) gọi `compute_metrics`:
  `src/bank_churn/evaluation/metrics.py` (L28–L37)
...
- **sklearn LogisticRegression**: tối ưu cho prediction/inference (dự đoán), dễ đóng gói thành pipeline và lưu `.joblib`.
  - Nơi ghép pipeline: `src/bank_churn/training/pipeline.py` (L29–L35)
  - Nơi build model: `src/bank_churn/model/logistic.py` (L22–L31)
- **statsmodels Logit**: tối ưu cho inference (suy luận thống kê) — cung cấp p-value, CI, summary rõ ràng cho báo cáo.
  - Nơi fit Logit + lấy CI/p-value: `src/bank_churn/stats/logit_odds_ratio.py` (L45–L52)
...
`src/bank_churn/preprocessing/pipeline.py` (L26–L33).

### 10.3 “OR của Balance gần 1” có nghĩa gì?

Trong OR table:

- `Balance` có OR ~ 1.0000026 (rất gần 1).

Điều này thường xảy ra vì:

- Đơn vị của `Balance` quá lớn (1 đơn vị tiền) → OR theo “1 đơn vị” nhỏ xíu.
...
- `src/bank_churn/evaluation/metrics.py` (L28–L30)

Tuy nhiên, với bài toán churn (mất cân bằng lớp) trong ngân hàng, threshold=0.5 **hiếm khi tối ưu**. Cần bàn luận theo business trade-off (đánh đổi kinh doanh):

- **Ưu tiên High Recall (không bỏ sót khách sắp churn)**:
  - Giảm threshold (ví dụ 0.3–0.4) ⇒ tăng TP nhưng cũng tăng FP.
  - Phù hợp khi chi phí bỏ sót khách churn (FN) cao hơn chi phí chăm sóc nhầm (FP).
...
- **Rescale biến liên tục để OR dễ hiểu**: `Balance` và `EstimatedSalary` có đơn vị lớn ⇒ OR theo “1 đơn vị” gần 1. Nên rescale theo 10k/100k để diễn giải.
